...
export function loadStore(wolfDir: string): AnatomyStoreData | null {
  try {
    const parsed = JSON.parse(
      fs.readFileSync(path.join(wolfDir, STORE_FILE), "utf-8"),
    );
    if (parsed && parsed.version === 1 && parsed.files && parsed.meta)
      return parsed as AnatomyStoreData;
    return null;
  } catch {
    return null;
  }
...
export function renderStore(store: AnatomyStoreData): string {
  const bySection = new Map<
    string,
    Array<{ file: string; entry: StoreFileEntry }>
  >();
  for (const [relPath, entry] of Object.entries(store.files)) {
    ...
    bySection
      .get(dir)!
      .push({ file: relPath.slice(relPath.lastIndexOf("/") + 1), entry });
  }
  const lines: string[] = [
    "# anatomy.md",
    "",
  ...
  for (const key of keys) {
    ...
    for (const { file, entry } of entries) {
      ...
      for (const sym of entry.symbols ?? []) {
        lines.push(
          `  - ${sym.kind} \`${sym.name}\` L${sym.startLine}-${sym.endLine} (~${sym.tokens} tok)`,
        );
      }
    }
    lines.push("");
...
export function loadStoreReconciled(
  wolfDir: string,
  projectRoot: string,
): AnatomyStoreData {
...
export function withAnatomyLock<T>(
  wolfDir: string,
  budgetMs: number,
  fn: () => T,
): T | null {
  ...
  while (true) {
    ...
    try {
      ...
      stale =
        ...
        Date.now() - body.acquiredAt > LOCK_STALE_MS;
      if (
        !stale &&
    ...
    if (stale) {
      const graveyard =
        lockPath + "." + crypto.randomBytes(4).toString("hex") + ".stale";
      try {
