...
function appendToMemory(
  ...
  try {
    ...
    const writeTokens = estimateTokens(
      fileContent || newStr,
      type as "code" | "prose" | "mixed",
    );
...
function trackSession(
  ...
  try {
    ...
    const editKey = normalizePath(
      path.relative(
        wolfDir.replace("/.wolf", ""),
        path.join(wolfDir.replace("/.wolf", ""), filePath),
      ),
    );
...
export function autoDetectBugFix(
  wolfDir: string,
  absolutePath: string,
  projectRoot: string,
  oldStr: string,
...
export function detectFixPattern(
  oldStr: string,
  newStr: string,
  ext: string,
  basename: string,
  ...
  if (newStr.includes("catch") && !oldStr.includes("catch")) {
    ...
    return {
      ...
      rootCause: "Code path had no error handling",
  ...
  if (
    ...
    return {
      ...
      rootCause: `No early return for: ${condition}`,
  ...
  if (newStr.includes("await ") && !oldStr.includes("await ")) {
    return {
      ...
      rootCause: "Async call without await",
