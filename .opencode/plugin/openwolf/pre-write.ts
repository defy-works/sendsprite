...
export function handlePreWrite(
  directory: string,
  sessionId: string,
  filePath: string,
  content: string,
...
function checkBugLog(
  ...
  const bugLog = readJSON<{ version: number; bugs: BugEntry[] }>(bugLogPath, {
    version: 1,
    bugs: [],
  });
  ...
  const fileMatches = bugLog.bugs.filter(
    (b: BugEntry) => path.basename(b.file) === basename,
  );
  ...
  const relevant = fileMatches.filter((bug: BugEntry) => {
    const tagHit = bug.tags.some((t: string) =>
      editText.includes(t.toLowerCase()),
    );
