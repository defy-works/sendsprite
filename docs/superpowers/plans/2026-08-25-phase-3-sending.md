...
- **Never self-enqueue from inside a handler on an `exclusive` queue — use a sweep.** pg-boss keeps one job per (queue, singletonKey) across created/retry/active (index `job_i6`); while a job is `active` its key is taken, so a retry it sends for itself is silently dropped. `domain.verify` and `webhook.deliver` are both driven by a cron sweep for this reason (review of Tasks 9–10 confirmation).
...
## Phase 3 status: COMPLETE (2026-08-25)

Tasks 1–16 shipped; the "As shipped" / "Review follow-ups" notes under each task record what changed after review, and `README.md` ("Sending"), spec §4/§7/§8/§12 and the e2e suite (`tests/e2e/send.spec.ts`: verify a domain with the fake SES, send via REST and SMTP, watch the log and overview) describe the shipped behaviour. Open these in the **Phase 4 plan**:

1. **OpenAPI from the shared zod schemas** (`SendEmailInput`, `EmailObject`, `EmailEventObject`, `ListQuery`, `ApiError`) → `/api/v1/openapi.json` + `/docs/api`; SDK contract tests against it (spec §7/§13).
2. **API-key-authenticated SSE.** `/api/stream` is session-scoped (`requireTeam()`); the CLI's `emails tail` needs a variant that accepts `Authorization: Bearer ss_live_…` (same `listenTeam` backend, filtered to `email.*` changes).
3. **`sendsprite/react` render → `html`.** The API has no `react` field; the SDK renders with `@react-email/render` (or `react-dom/server`) and posts `html` (+ optional `text`). Keep `template`/`variables` reserved for Phase 5.
4. **SDK retry policy.** Retry `429` (`rate_limited`, `daily_quota_exceeded` is not retryable within the day) and `5xx` with jittered backoff, honouring `x-ratelimit-*`; never retry a `POST /emails` without an `idempotencyKey` (the server's replay returns 200 with the earlier id).
5. **`worker.ts` in the image / `WORKER_MODE=separate` decision** (carried from Phase 2): the standalone image has no `bun run worker`; either trace `src/worker.ts` into the image or drop `separate` from the docs. Heartbeat-based worker health is done (T15) — the web tier can already report a separate worker.
6. **Webhook retry-sweep granularity.** The sweep runs every minute, so the 1 m step lands at 1–2 m; if finer timing matters, use pg-boss `startAfter` sends from the sweep (keyed per delivery) instead of the cron tick.
7. **SMTP PROXY protocol** (`smtp-server` `useProxy`) so login throttling per remote IP works behind a load balancer; today the README says to expose 587 directly.
...
**`fix(web): domain re-check keeps verified on transient SES status; SMTP delivers to envelope recipients`.** (6) The daily forced re-check could take a working domain offline: a `PENDING`/`TEMPORARY_FAILURE` from `GetEmailIdentity` (SES re-polling DNS) demoted it to `pending`, and because `verifyUntil` is only reset by Re-verify, a verified domain older than 72 h went straight to `failed`. `verifyDomain` now never applies the 72 h `expired` rule to a domain that was `verified`, demotes from `verified` only when SES reports `FAILED`/`NOT_STARTED` (or the identity is gone), and on a transient status keeps `verified` + `verifiedAt`, bumps `lastCheckedAt` and sets `lastError: "SES reported <status>; will re-check"` (cleared by the next agreeing check). `domains.test.ts`: verified + past `verifyUntil` + `TEMPORARY_FAILURE` stays verified with that `lastError`; `FAILED` demotes to `pending` and fans out `domain.failed` (the fixtures that used `PENDING` to mean "SES disagrees" now use a `FAILED` identity). (7) The SMTP relay sent to header addresses rather than the envelope: a client relaying a message with `To: customer@x` but `RCPT TO:<me@x>` would have mailed the customer. `handleInbound` now takes recipients from `rcptTo` only — `to = envelope ∩ header To`, `cc = envelope ∩ header Cc`, `bcc = envelope − (to ∪ cc)`, and when no envelope address is in the To header every non-Cc envelope recipient is `to` (Bcc-only sends). `smtp.test.ts`: `To: customer@x.io` with envelope `[me@x.io]` → sent to `me@x.io` only.

**`fix(web): resolve-and-pin webhook targets against private ranges; sending-only keys cannot override suppression`.** (8) The webhook URL check was syntactic, so a public name resolving to a private address (DNS rebinding, a hijacked CNAME) let the worker POST signed payloads at internal services. `deliver()` now calls `resolveTarget`: `dns.promises.lookup(host, { all: true })`, every answer vetted with the new `isPublicIp` (`lib/url-safety.ts`, the IP-only half of `isPublicHttpUrl`); any private answer records the attempt with excerpt `PRIVATE_TARGET` ("target resolves to a private address") and marks the delivery `exhausted` (no retry, `failingSince` still starts); a resolver error is an ordinary retried failure (`dns: …`). The request then goes through `pinnedFetch`: undici's `fetch` with an `Agent({ connect: { lookup } })` whose lookup answers only the vetted addresses (TLS still verifies the hostname; undici's own `fetch` because Bun's global fetch ignores `dispatcher`; `undici` added as a direct dependency). Tests: `url-safety.test.ts` (`isPublicIp`), `webhooks.test.ts` (mocked `dns.promises.lookup` → `127.0.0.1` among the answers → exhausted without a fetch; public → fetched; `ENOTFOUND` → pending), and the pg-boss loop test stubs `undici.fetch` + the resolver. (9) `overrideSuppression` was honoured for any key. `SendContext.permission` (set from `auth.key.permission` in `api/v1/emails/_shared.ts` and the SMTP `onAuth` → `SmtpUser`; absent for dashboard sends) is checked in `createEmail`: a `sending_only` key's `overrideSuppression` is ignored. README notes both. `emails.test.ts` covers the override refusal.

**`fix(web): touch updated_at on throttle/defer; demotion gets a fresh 72 h window; opaqueredirect handling`** (re-verification). (a) The throttled and deferred branches of `sendQueuedEmail` left `updated_at` alone, so a row a worker was actively re-queueing looked orphaned to `sweepQueuedEmails` after 5 minutes; both now `touch` the row (`UPDATE emails SET updated_at = now()`), and the sweep is capped at `SWEEP_BATCH = 5000` rows per tick. Test: a throttled and a deferred row are not selected by `sweepQueuedEmails(now + 4 min)`. (b) Demoting a verified domain to `pending` sets `verifyUntil = now + 72 h` — the window set at creation has long passed, and the next sweep tick would have marked the domain `failed` at once; the sweep test now demotes with a past `verifyUntil`, checks the fresh window and runs the following tick, which keeps it `pending`. (c) `deliver()` also treats a spec-style `opaqueredirect` (status 0) as "redirect not followed"; in practice undici 8 under `redirect: "manual"` hands the 3xx itself back (the real-connection test observes `statusCode: 302`), so the existing 3xx branch is what fires and the `opaqueredirect` check is defensive. New `webhooks.test.ts` case runs the real pinned path (`dns.promises.lookup` → `127.0.0.1`, `isPublicIp` let through via a partial mock) against a local `http.createServer`: 302 → pending with the excerpt and the redirect target never requested; 200 → delivered. This also exercises the undici `Agent` lookup pin and the dispatcher `close()` ordering.

**Additional Phase 4 openers from the final integration review (2026-08-25):** unify service failure shapes (`lib/result.ts` `code` typed as `ErrorCode`; one `serviceFailure()` mapper in `api-response.ts` instead of per-route ad-hoc mapping); cursor pagination for `/api/v1/{suppressions,api-keys,domains,webhooks}` lists; make `packages/shared` consumable by the SDK (public, real dual ESM/CJS build, `node:crypto`-free root barrel with a separate `@sendsprite/shared/node` entry for webhook signing; shared zod contracts for domains/api-keys/webhooks/suppressions inputs and public shapes; `PATCH /emails/:id` body schema; `EmailEventObject.type` enum); audit rows for email cancel/resend/reschedule via `TeamActor`; REST audit ip/UA; consistent audit action naming; `email_events` payload PII purge in retention; body caps on non-email routes; SNS `Timestamp` freshness; `email.send` worker `batchSize`/`localConcurrency` sized from `MaxSendRate`; align shutdown timeout with SES request timeout; `domain.provision` singletonKey.
...
Write the sketched functions fully. Handler `webhook-deliver.ts`: `registerQueue<{deliveryId}>(Q.webhookDeliver, jobs → deliver(id, { enqueue }), { queue: { policy: "exclusive", retryLimit: 0, expireInSeconds: 60 } })` (retries are our own schedule). Every `webhook.deliver` enqueue passes `singletonKey: deliveryId` so a Replay is deduped against a queued retry, and `deliver()` also skips rows already `delivered`. **`deliver()` never enqueues its own retry** (under `exclusive` the insert would be dropped while its job is `active`): on failure it only writes `attempt`, `status: "pending"`, `nextRetryAt`; the cron `webhook.retry-sweep` (`Q.webhookRetrySweep`, `* * * * *`, `retryLimit: 0`, exported as `sweepWebhookRetries(now)`) selects `status='pending' AND next_retry_at <= now()` and enqueues each with `singletonKey`. Initial fan-out and Replay still enqueue directly; Replay also sets `nextRetryAt = now` so a deduped send is picked up by the next sweep. `tests/integration/webhook-loop.test.ts` runs this through real pg-boss (fail → sweep → attempt 2 → fix → sweep → delivered). Integration `maxWorkers: 4` caps concurrent embedded Postgres instances.

**Hardening (review of Tasks 9–10, shipped as `fix(web): webhook target validation, manual redirects, capped responses, replay race`, then `fix(web): sweep-driven webhook retries (no self-enqueue under exclusive policy); cap integration parallelism`):**

- Target validation: `lib/url-safety.ts` `isPublicHttpUrl(url, { httpsOnly })` (pure, unit-tested) rejects `localhost`, `*.localhost`, `*.local`, `*.internal`, single-label hosts, credentials in the URL, and literal IPs in loopback / RFC 1918 / link-local (incl. `169.254.169.254`) / CGNAT / `0.0.0.0/8` / multicast-reserved / `::1` / `::` / ULA / IPv6 link-local / IPv4-mapped forms. The zod `url` refine uses it (message "Webhook URL must be a public https address"; https is mandatory only in production). DNS-rebinding-safe resolution (resolve, check, connect to the checked address) shipped in the integration review fixes: `deliver()` resolves the host, vets every answer with `isPublicIp` and pins the connection via undici.
...
As shipped: `loadOrGenerateCert` is async (selfsigned 5 is promise-only, `notAfterDate` instead of `days`); `server.ts` adds a per-process login throttle (5 failures / 10 min per remote address → 535 "Too many failed logins" for 10 min; capped at 10k entries, oldest evicted; per remote IP, so a proxy without PROXY protocol shares one counter — documented in README); `monthly_quota_exceeded` → 452 and `forbidden` → 550 join the mapping; body-less MIME → 501; missing subject → `(no subject)`; recipients come from the envelope (`to = rcptTo ∩ header To`, `cc = rcptTo ∩ header Cc`, `bcc = rcptTo − (to ∪ cc)`; a header address not in RCPT TO is never sent to — see "Integration review fixes"); the DATA stream is copied through a bounded `PassThrough` (`boundedBody`) so the parser never holds more than `SMTP_MAX_SIZE` — on overflow the input is drained and 552 sent; `maxClients: 50`, `closeTimeout: 5000`; AUTH only after STARTTLS unless `SMTP_ALLOW_INSECURE_AUTH=true` (new env, default false, dev only); `session.smtpUser` via a `declare module "smtp-server"` augmentation; compose maps `${SMTP_PORT:-587}` on both sides because `.env` is passed through to the container, and the `worker` profile sets `SMTP_ENABLED=false`; `instrumentation.ts` starts the relay in every `WORKER_MODE`, loads the cert outside the try so a bad `SMTP_TLS_CERT/KEY` path aborts boot, and only `EADDRINUSE` is logged-and-continue.
...
- [x] **Step 1: Retention (TDD)** — `purgeOldBodies(retentionDays, now, batch = 500)` nulls `html/text`, deletes `email_attachments`, sets `bodyPurgedAt` for emails older than N days not yet purged, looping batches until none remain; it also deletes `webhook_deliveries` rows older than the same window (as shipped: returns `{ emails, deliveries }` counts). Cron `15 3 * * *` (`Q.retentionPurge`, `retryLimit: 0`) reads `instance_settings.retention_days`. As shipped: `EMAIL_RETENTION_DAYS` was never read anywhere, so it is removed from `env.schema.ts`, `.env.example`, `install.sh` and the README — Settings → Instance is the single source of the window (README says so). Test: old vs new email with attachments → old purged (metadata kept), new intact, idempotent; batch loop; old deliveries deleted; job reads the configured days.
- [x] **Step 2: Heartbeat persistence** — `heartbeat.ts` upserts `worker_heartbeats(process_id = os.hostname()+":"+pid, last_seen_at = now())` and prunes rows older than a day; cron stays `*/5`. `health.ts` `collect()` adds `workerLastSeenSeconds` (min age across rows, null when none) and reports `worker: "running"` if any row is < 10 min old even when this process is `disabled` (separate-worker deployments); `degraded` when `WORKER_MODE !== "none"`, no in-process worker, and no heartbeat < 15 min (so a fresh instance with no worker yet reports `degraded`, still HTTP 200). Unit cases in `health.test.ts`; integration in `worker.test.ts`.
- [x] **Step 3: Verified re-check** — `selectSweepCandidates()` returns `{ id, force }` and includes verified rows whose `lastCheckedAt` is > 24 h old with `force: true`; the sweep puts `force` in the job data and the `domain.verify` handler passes it to `verifyDomain(id, { enqueue }, { force })`. `verifyDomain`/`reverifyDomain` now take `enqueue` (callers: server action, REST verify route, handler) and fan out via `fanOutEvent` with `{ domain: publicDomain(d) }`: `domain.verified` on any flip to verified, `domain.failed` when a verified domain is demoted or a check marks a domain `failed` (timeout, identity gone). A forced check demotes a verified domain only on SES `FAILED`/`NOT_STARTED` (or NotFound); transient statuses keep it verified (see "Integration review fixes"). Tests in `domains.test.ts`.
- [x] **Step 4: Audit action names** — `updateInstanceSettings(patch, actor?, { audit?, action? })`; `aws-connect` uses `aws.connect` / `aws.disconnect` / `ses.production.request` (always writes a row, even if unchanged — `refreshSesAccount(actor, { action })`) / `ses.account.refresh` (only on change); `cloudflare-connect` `cloudflare.connect`/`cloudflare.disconnect`; settings form and setup keep `instance.update`. Tests updated (`aws-connect`, `cloudflare-connect`, `instance-settings`).
- [x] **Step 5: Commit** → `feat(web): retention purge, worker heartbeat health, verified-domain re-check, audit action names`.
...
- [x] **Step 1: e2e** — after the existing `setup` project: create API key in `/app/api-keys` (copy secret from the `CopyField` text), `POST /api/v1/emails` via `page.request` with the key from `mail.<suffix>` … the domain from `setup.spec.ts` is `pending`, so first mark it verified directly? Not possible from e2e — instead the fake SES `GetEmailIdentity` returns `SUCCESS` when `process.env.AWS_E2E_VERIFY === "1"` (set in playwright webServer env) so the sweep verifies the domain within ~2 min — too slow; alternative: add an internal, dev-only route? Simplest: e2e uses the **Instance settings** path to add the domain, then in the spec waits for `Re-verify` and clicks it (inline verify → with the fake returning SUCCESS the domain becomes `verified` immediately). Then send via API → expect 201 → open `/app/emails` → row visible with status `sent` (worker inline + fake SES) → open detail → events show `queued`, `sent`. Also SMTP: `SMTP_ENABLED=true` with `SMTP_PORT=2587` in the webServer env; use `nodemailer` inside the spec to send one message and assert it appears.
- [x] **Step 2: Docs** — README: "Sending" section (API key, curl example, batch, scheduling, idempotency, attachments, tracking, webhooks + signature verification snippet, SMTP relay config incl. TLS note, suppressions, limits/headers), env rows (`SMTP_PORT`, `SMTP_TLS_CERT/KEY`, `SMTP_MAX_SIZE`; retention env removed/clarified). Spec §4/§7/§8 "as shipped" notes (SES tracking disabled in favour of own; webhook disable email deferred; REST templates/contacts in Phase 5). Plan: "Phase 3 status: COMPLETE" block with Phase 4 openers (SDK-facing: OpenAPI generation from shared zod, `sendsprite/react` render arrives as `html`, CLI `emails tail` uses `/api/stream` with API key auth — needs an API-key-authenticated SSE variant).
- [x] **Step 3: Commit** → `test(web): e2e send via API and SMTP; docs for sending`.

**As shipped:** `send.spec.ts` (project `app`, after `setup`) signs up its own owner + team, adds `mail.e2e-send-<suffix>.com`, waits for the records, clicks **Re-verify** (inline `verifyDomain`; the fake SES answers `SUCCESS` for DKIM and MAIL FROM when `AWS_E2E_VERIFY=1`, set in `playwright.config.ts`'s webServer env) and expects `verified`; creates an API key and reads the secret from the CopyField's `code.select-all`; `POST /api/v1/emails` → 201 `{ id }` with `x-ratelimit-limit`; the row shows in `/app/emails` and the detail's Events tab shows "Queued" and "Sent to SES" (reload up to 30 s; fake `SendEmailCommand` → `fake-<boot nonce>-<n>`, unique because `emails.ses_message_id` is unique and the dev database persists between runs); then one `nodemailer` message to `127.0.0.1:2587` (`ignoreTLS`, API key as password; webServer env sets `SMTP_ENABLED=true`, `SMTP_PORT=2587`, `SMTP_ALLOW_INSECURE_AUTH=true`) — the relay answers a plain 250, so the row is found by its unique subject, its detail shows `smtp` as Source and the same two events; the overview "Sent · 24 h" tile is ≥ 2; the domain is deleted through the UI at the end. CI needs no change: the webServer env is unconditional and 2587 is free on the runner. Templates in `createEmail` are still a Phase 5 item, so the docs say the `template` field is accepted but not yet honoured.
