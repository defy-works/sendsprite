...
describe("campaign CRUD", () => {
  ...
  it("refuses a header layout from another team, and links one of its own", async () => {
    ...
    // The refusal names the slot that carried the bad id, not always the header.
    const foreignFooter = await (
      await svc()
    ).createCampaign(mine.actor, {
      ...mine.draft,
      footerLayoutId: theirLayout,
    });
    expect(foreignFooter).toMatchObject({
      ok: false,
      code: "validation_error",
      details: { field: "footerLayoutId" },
    });
