...
import { signUnsubscribeToken } from "@sendsprite/shared/node";
import { db } from "@/db";
import {
  campaignRecipients,
  campaigns,
...
export async function fanoutChunk(
  ...
  const planned: Planned[] = contacts.map((contact) => {
    ...
    const url = unsubscribeUrl(contact.id, campaign.id, env);
    return {
      contact,
      emailId,
      row: {
        ...
        ...body(campaign, emailId, url, tracking),
...
const unsubscribeUrl = (
  contactId: string,
  campaignId: string,
  env: { APP_URL: string; APP_SECRET: string },
) =>
  `${env.APP_URL}/unsubscribe/${signUnsubscribeToken(contactId, campaignId, env.APP_SECRET)}`;

/**
 * RFC 8058's one-click pair.
 *
 * Without these Gmail and Outlook show no native unsubscribe button, and the
 * recipient's only route out is the spam button — which costs far more
 * reputation than an unsubscribe does. `List-Unsubscribe-Post` is what tells a
 * conforming client to POST rather than follow the link, which is the whole
 * reason `GET /unsubscribe/:token` is allowed to be a confirmation page.
 *
 * Both values are header-safe by construction rather than by hope: the name
 * matches `HEADER_NAME` (`[A-Za-z0-9-]{1,80}`), `list-unsubscribe` is
 * explicitly not in the reserved set, and the value satisfies
 * `NO_CONTROL_CHARS` because a token is base64url (`[A-Za-z0-9_-]`) and
 * `APP_URL` is a parsed `z.url()`. The fan-out inserts `emails` rows directly,
 * so `SendEmailInput` never sees these — the guarantee has to hold here.
 */
const listUnsubscribeHeaders = (url: string): Record<string, string> => ({
