...
async function checkRefs(
  ...
  if (wanted.length) {
    ...
    // `layoutIds` is positional — `[header, footer]` — so the refusal can
    // name the slot that carried the bad id.
    const missingAt = layoutIds.findIndex((id) => id !== null && !have.has(id));
    if (missingAt !== -1)
      return {
        ok: false,
        ...
        details: {
          field: missingAt === 0 ? "headerLayoutId" : "footerLayoutId",
        },
      };
  }
  const [book] = await db()
