    ...
    <div ref={root} className={cn("relative", className)}>
      ...
      {open && (
        <ul
          ...
          tabIndex={-1}
          className={cn(
            // At least as wide as the trigger, but sized to the options: a
            // trigger showing "Full" is narrow, and a menu that inherited
            // that width truncated "Sending only" and wrapped its hint one
            // word per line. Capped so a long hint wraps instead of pushing
            // the menu off a phone screen.
            "popover absolute z-50 mt-1 max-h-72 w-max min-w-full max-w-[min(20rem,calc(100vw-2rem))] overflow-y-auto p-1 shadow-glass",
              ...
              <li key={o.value} role="presentation">
                ...
                >
                  ...
                  <span className="flex min-w-0 flex-col">
                    ...
                    {o.hint && (
                      <span className="text-xs text-pretty text-white/50">
                        {o.hint}
                      </span>
                    )}
                  </span>
